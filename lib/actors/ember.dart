...
class EmberPlayer extends SpriteAnimationComponent with KeyboardHandler, CollisionCallbacks, HasGameReference<EmberQuestGame> {
  final Vector2 velocity = Vector2.zero(), fromAbove = Vector2(0, -1);
  final double gravity = 15, jumpSpeed = 600, moveSpeed = 200, terminalVelocity = 150;
  int horizontalDirection = 0;
  bool hasJumped = false, isOnGround = false, hitByEnemy = false;

  EmberPlayer({required super.position}) : super(size: Vector2.all(64), anchor: Anchor.center);

  @override
  Future<void> onLoad() async {
    animation = SpriteAnimation.fromFrameData(game.images.fromCache('ember.png'), SpriteAnimationData.sequenced(amount: 4, textureSize: Vector2.all(16), stepTime: 0.12));
    add(CircleHitbox());
  }
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    if (other is GroundBlock || other is PlatformBlock) {
      if (intersectionPoints.length == 2) {
        ...
        final double separationDistance = (size.x / 2) - collisionNormal.length;
  ...
  void hit() {
    ...
    add(OpacityEffect.fadeOut(EffectController(alternate: true, duration: 0.1, repeatCount: 5))..onComplete = () => hitByEnemy = false);
  }
}
